...
extract_top_model(version = "v2", sp = "lionsmane") # v2.014, rf
extract_top_model(version = "v2", sp = "moon") # v2.034, rf
extract_top_model(version = "v2", sp = "whitecross") # v2.058, rf
extract_top_model(version = "v2", sp = "comb") # v2.078, nn
...
glimpse(all) # 958 rows

obs <- all %>% 
  filter(month %in% c("May", "Jun", "Jul", "Aug", "Sep"),
         type %in% c("lionsmane", "moon", "whitecross", "comb"))
  
glimpse(obs) # 826 rows
...
# adding TSS to results
```{r}
cfg = read_config("data/versions/v5/v5.001/v5.001.yaml")
target_date = as.Date("2025-08-19")
```
