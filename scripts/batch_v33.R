...
# date: 2025-07-22
# usage:
# Rscript scripts/batch_v33.R \
# --dates 2025-07-20 2025-07-25 \
# --configs data/versions/v3/v3.001/v3.001.yaml data/versions/v3/v3.003/v3.003.yaml
...
main <- function(start_date, end_date, start_config, end_config) {
  model_script <- "/mnt/s1/projects/ecocast/projects/gom-jellycast-dev/scripts/model_v3.R"
  ...
  for (config in configs) {
    ...
    for (d in dates) {
      ...
      if (exit_code != 0) {
        warning(sprintf("model_v3.R returned non-zero exit code for date %s", format(d, "%Y-%m-%d")))
      }
    }
  }
  
  extract_summary(version = "v3")
  invisible(0)
}
