...
# example:
# Rscript scripts/model_v3.R data/versions/v3/v3.001/v3.001.yaml 2025-06-01
# 
# params: 
...
main <- function(cfg, target_date) {
  root <- cfg[["root"]]
  region <- cfg[["region"]]
  poly <- read_coastline_buffer(buffer = 100000,
                                     path = "/mnt/s1/projects/ecocast/projects/gom-jellycast-dev/data/polygons")
  species <- cfg[["obs"]][["type"]]
  duration <- cfg[["duration"]]  # "weekly" in this config
  variables <- unlist(cfg[["predictors"]][["phy"]])
  version   <- cfg[["version"]]
  product   <- cfg[["product"]]
  model     <- cfg[["model"]]
  random_bkg<- isTRUE(cfg[["obs"]][["random"]])
  min_obs_threshold <- cfg[["obs"]][["min"]]  # For example, 3
  ...
  # Get background and observation points
  ...
  # Filter points for the specific day and weekly window
  day_obs <- obs %>% filter(date == target_date)
  day_bkg <- bkg %>% filter(date == target_date)
  
  # Create a 7-day window (centered on target_date)
  start_week <- target_date - 3
  end_week <- target_date + 3
  week_obs <- obs %>% filter(date >= start_week & date <= end_week)
  week_bkg <- bkg %>% filter(date >= start_week & date <= end_week)
  
  # Record summary information based on the day (for plotting and evaluation)
  ...
  # Save the original distribution plot if we have any points for this day
  ...
  # Read static deptho data if needed
  ...
  # Write data frame for training using the WEEKLY window
  df <- write_df(x, week_obs, week_bkg)
  prep <- prep_split(seed = cfg$obs$seed %||% 123, data = df)
  train <- prep$train
  test  <- prep$test
  
  # Define a helper function to check if there's sufficient data
  sufficient_data <- function() {
    (nrow(week_obs) >= min_obs_threshold) && (nrow(week_bkg) >= min_obs_threshold)
  }
  ...
  if(isTRUE(model$maxent)) {
    if(sufficient_data()) {
      ...
      charlier::info("not enough data to run maxent model")
    }
  }
  ...
  if(isTRUE(model$rf)) {
    if(sufficient_data()) {
      ...
      charlier::info("not enough data to run random forest model")
    }
  }
  
  ### Boosted Regression Trees Model (BRT) ###
  if(isTRUE(model$brt)) {
    if((nrow(week_obs) >= 8) && (nrow(week_bkg) >= 8)) {
      ...
      brt_pred <- x[1]
      brt_pred[[1]] <- matrix(df_covs$pred, nrow = dim(x)[1], ncol = dim(x)[2])
      names(brt_pred) <- "predicted_distribution"
      if (!is.null(cfg[["polygons"]])) {
      ...
      charlier::info("not enough data to run BRT model")
    }
  }
  ...
  if(isTRUE(model$glm)) {
    if(sufficient_data()) {
      ...
      glm_pred <- x[1]
      glm_pred[[1]] <- matrix(df_covs$pred, nrow = dim(x)[1], ncol = dim(x)[2])
      names(glm_pred) <- "predicted_distribution"
      if (!is.null(cfg[["polygons"]])) {
      ...
      charlier::info("not enough data to run GLM model")
    }
  }
  ...
  if(isTRUE(model$nn)) {
    if(sufficient_data()) {
      ...
      nn_pred <- x[1]
      nn_pred[[1]] <- matrix(df_covs$pred, nrow = dim(x)[1], ncol = dim(x)[2])
      names(nn_pred) <- "predicted_distribution"
      if (!is.null(cfg[["polygons"]])) {
      ...
      charlier::info("not enough data to run neural network model")
    }
  }
  
  # Update summary CSV (merging if already exists)
...
# Source environment setup if needed
source("/mnt/s1/projects/ecocast/projects/gom-jellycast-dev/setup.R")

# Parse input arguments
Args <- argparser::arg_parser("Weekly model builder and predictor", name = "model_v3.R") %>%
  argparser::add_argument("config", help = "path to config file") %>%
  argparser::add_argument("date", help = "forecast target date (YYYY-MM-DD)") %>%
  argparser::parse_args()
cfg <- yaml::read_yaml(Args$config)
target_date <- as.Date(Args$date)

# Set working directory to the root provided in the config
setwd(cfg[["root"]])
