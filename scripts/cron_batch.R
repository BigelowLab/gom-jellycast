...
script <- sprintf("batch_v23.R --dates %s %s", format(Sys.Date(), "%Y-%m-%d"), format(Sys.Date() + 8, "%Y-%m-%d"))
args <- sprintf("%s/scripts/%s --configs %s/data/versions/v2/v2.001/v2.001.yaml %s/data/versions/v2/v2.080/v2.080.yaml", path, script, path, path)
